...
tickers = c('SPY','^VIX','^TNX','OIL','GOLD','^N225','^FTSE',  'GLD', 'SPXS', 'SPXL');
...
#CM = NULL;
	...
	STATS = rbind(STATS, colMeans(STATS));
	print(STATS);
	#colnames(CM) = tickers;
	#plot_cumsum(CM);
}
...
jpeg(filename = "plots1.jpeg", width = 280, height = 280, units = "px", bg = "white");
plot(PCARot[,1], PCARot[,2], type= 'p', pch = 20, cex = 1, col = 'red', xlab = 'PC1', ylab = 'PC2')
text(PCARot[,1], PCARot[,2], labels = rownames(PCARot));
dev.off();
jpeg(filename = "plots2.jpeg", width = 280, height = 280, units = "px", bg = "white");
plot(PCARot[,1], PCARot[,3], type= 'p', pch = 20, cex = 1, col = 'red', xlab = 'PC1', ylab = 'PC3')
text(PCARot[,1], PCARot[,3], labels = rownames(PCARot));
dev.off();
jpeg(filename = "plots3.jpeg", width = 280, height = 280, units = "px", bg = "white");
plot(PCARot[,2], PCARot[,3], type= 'p', pch = 20, cex = 1, col = 'red', xlab = 'PC2', ylab = 'PC3')
text(PCARot[,2], PCARot[,3], labels = rownames(PCARot));
dev.off();
t5load = myPCA$x[,1:5];
return(t5load);
}
